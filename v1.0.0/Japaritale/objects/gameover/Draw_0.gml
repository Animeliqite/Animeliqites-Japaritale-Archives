...
draw_sprite_ext(spr_bg_game_over,0,0,0+floor(bgY),2,2,0,c_white,bgAlpha);
draw_sprite_ext(spr_char_kaban_lie,0,320+playerX,360+playerY+floor(bgY),2,2,0,c_white,bgAlpha);
draw_sprite_ext(spr_bg_game_over_vignette,0,0,0,2,2,0,c_white,1);
	...
	if(textAlpha2==1&&!fading){
		...
		if(Input_IsPressed(INPUT.CONFIRM)){
			if(selection==0){
				...
				fader.color=c_white;
				Fader_Fade(0,1,120);
				Object_Shake(4,"playerX",id,0,true,1);
				Object_Shake(4,"playerY",id,0,true,1);
				alarm[5]=120;
				alarm[6]=30;
			}
			if(selection==1){
		...
		if(selection==0) draw_sprite_ext(spr_battle_soul_red,0,140,420,1,1,0,c_white,1);
		if(selection==1) draw_sprite_ext(spr_battle_soul_red,0,410-string_width(Lang_GetString("gameover.give_up")),420,1,1,0,c_white,1);
	}
