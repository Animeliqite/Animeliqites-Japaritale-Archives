...
if(_menu==0){
	...
	if (_mode_copy){
		...
		if (!instance_exists(_inst_header_copy)) {
			...
			_inst_header_copy.text=_prefix+Lang_GetString("menu.file.copy");
			_inst_header_copy.depth=DEPTH_UI.TEXT;
		}
	}else if(_mode_erase) {
		...
		if (!instance_exists(_inst_header_erase)) {
			...
			_inst_header_erase.depth=DEPTH_UI.TEXT;
		}
	}else{
		instance_destroy(_inst_header_copy);
		instance_destroy(_inst_header_erase);
		if (!instance_exists(_inst_header)) {
			...
			_inst_header.depth=DEPTH_UI.TEXT;
		}
	}
...
if(_menu==1||_menu==3){
	if(!instance_exists(_inst_naming_title)){
		...
		_inst_naming_title.depth=DEPTH_UI.TEXT;
	}
	if(!instance_exists(_inst_naming_letters)){
		...
		_inst_naming_letters.depth=DEPTH_UI.TEXT;
	}
	if(!instance_exists(_inst_naming_backspace)){
		...
		_inst_naming_backspace.depth=DEPTH_UI.TEXT;
	}
	if(!instance_exists(_inst_naming_done)){
		...
		_inst_naming_done.depth=DEPTH_UI.TEXT;
	}
	with(text_typer){
		event_user(15);
	}
