...
function Battle_SetMenu() {
	...
	if(MENU==BATTLE_MENU.ITEM_ACTION){
		Battle_SetMenuChoiceItemAction(0,false);
		instance_create_depth(0,0,0,battle_menu_item_scrollbar);
	}
	...
	if(CALL){
		if (USE_ENEMY_CHOICE)
			Battle_CallEnemyEvent(BATTLE_ENEMY_EVENT.MENU_SWITCH,ENEMY_CHOICE);
		else Battle_CallEnemyEvent(BATTLE_ENEMY_EVENT.MENU_SWITCH);
	}
