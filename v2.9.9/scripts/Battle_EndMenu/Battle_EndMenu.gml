function Battle_EndMenu() {
	...
	if(Battle_GetState()==BATTLE_STATE.MENU){
		...
		if(BUTTON==BATTLE_MENU_CHOICE_BUTTON.ITEM){
			battle._menu_item_used_last=Item_Get(Battle_GetMenuChoiceItem(),battle._menu_item_slot_type[Battle_GetMenuChoiceItem()]);
			Item_CallEvent(Item_Get(Battle_GetMenuChoiceItem(),battle._menu_item_slot_type[Battle_GetMenuChoiceItem()]),ITEM_EVENT.USE,Battle_GetMenuChoiceItem());
		}
		...
		//调用事件
		Battle_CallEnemyEvent(BATTLE_ENEMY_EVENT.MENU_END,ENEMY_CHOICE);
