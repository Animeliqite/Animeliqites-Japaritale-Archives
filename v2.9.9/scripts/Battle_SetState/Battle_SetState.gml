...
function Battle_SetState() {
	...
	if(STATE==BATTLE_STATE.TURN_PREPARATION){
        if(instance_exists(battle._handler_obj)){
            with(battle._handler_obj){
                event_user(8);
            }
        }
        
		if(battle_enemy._enemy_button_fade){
			Anim_Create(battle_button,"image_alpha",0,0,battle_button.image_alpha,0.5,15,0,-1,-1,ANIM_MODE.ONESHOT,false);
		}
		...
		var MOVE_SPEED=Battle_GetTurnInfo(BATTLE_TURN.BOARD_MOVE_SPEED,15);
		var MOVE_DURATION=Battle_GetTurnInfo(BATTLE_TURN.BOARD_MOVE_DURATION,15);
		...
		var SIZE_SPEED=Battle_GetTurnInfo(BATTLE_TURN.BOARD_SIZE_SPEED,15);
		var SIZE_DURATION=Battle_GetTurnInfo(BATTLE_TURN.BOARD_SIZE_DURATION,15);
	...
	if(STATE==BATTLE_STATE.IN_TURN){
		if(battle_enemy._enemy_button_fade){
			Anim_Create(battle_enemy,"image_alpha",0,0,battle_enemy.image_alpha,0.5,15,0,-1,-1,ANIM_MODE.ONESHOT,false);
		}
	...
	if(STATE==BATTLE_STATE.BOARD_RESETTING){
		if(battle_enemy._enemy_button_fade){
			Anim_Create(battle_button,"image_alpha",0,0,battle_button.image_alpha,1,15,0,-1,-1,ANIM_MODE.ONESHOT,false);
			Anim_Create(battle_enemy,"image_alpha",0,0,battle_enemy.image_alpha,1,15,0,-1,-1,ANIM_MODE.ONESHOT,false);
		}
		...
		var MOVE_SPEED=Battle_GetTurnInfo(BATTLE_TURN.BOARD_RESET_MOVE_SPEED,15);
		var MOVE_DURATION=Battle_GetTurnInfo(BATTLE_TURN.BOARD_RESET_MOVE_DURATION,15);
		...
		var SIZE_SPEED=Battle_GetTurnInfo(BATTLE_TURN.BOARD_RESET_SIZE_SPEED,15);
		var SIZE_DURATION=Battle_GetTurnInfo(BATTLE_TURN.BOARD_RESET_SIZE_DURATION,15);
