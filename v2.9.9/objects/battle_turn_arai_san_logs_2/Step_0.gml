if(phase==0){
	...
	Anim_Create(_trash_can,"x",ANIM_TWEEN.SINE,ANIM_EASE.IN_OUT,_trash_can.x+40,_trash_can.x-40,30,75,-1,-1,ANIM_MODE.PINGPONG,false);
	alarm[0]=45;
	phase=1;
}
...
if(phase==2){
	...
	var chances = 0;
	Battle_CreateLogBullet(battle_board.x+battle_board.right+20,battle_board.y+battle_board.down+10,-5,0,1/2,(chances == 0 ? c_white : c_aqua));
	Battle_CreateLogBullet(battle_board.x-battle_board.left-20,battle_board.y+battle_board.down+10,5,0,1/2,(chances == 0 ? c_white : c_aqua));
	alarm[0]=room_speed;
	phase=3;
}
