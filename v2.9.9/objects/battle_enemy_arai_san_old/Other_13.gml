...
switch(Battle_GetMenu()){
	...
	case BATTLE_MENU.FIGHT_DAMAGE:
		_fight_phase++;
		var dmg=Battle_GetMenuFightDamage()*2;
		var hp_orig=_hp;
		
		_hp=(_hp-dmg >= 0 ? _hp-dmg*(_fight_tries+1) : 0);
		
		//Create damage number
		var inst=instance_create_depth(x,y-150,0,battle_damage);
		inst.damage=dmg*(_fight_tries+1);
		...
		if(dmg>0){
			_face_expression=1;
			audio_play_sound_on(global._gmu_emitter_sfx,snd_damage,0,false);
