...
draw_sprite_ext(sprite_index, image_index, x + jitterAmountX, y + jitterAmountY, image_xscale, image_yscale, image_angle, image_blend, image_alpha);
...
jitterProbability = random(100);
if (jitterProbability >= 100 - (healthbarAmount / 4)) {
	if (!partCreatedOnce) {
		if (alarm[1] < 0) alarm[1] = 1;
		partCreatedOnce = true;
	}
	jitterAmountX = round(random(1));
	jitterAmountY = round(random(1));
}
else {
