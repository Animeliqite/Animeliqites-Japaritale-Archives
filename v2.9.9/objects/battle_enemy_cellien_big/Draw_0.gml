...
if (!surface_exists(_surf_tentacle)) {
	...
	for (var rot = 0; rot <= 180; rot += 45) {
		var VAR_ROT=variable_instance_get(id,"_rot_add_" + string(rot)),
			X1=(x + (_body_x / 2) - lengthdir_x(sprite_width / 2, -rot + (VAR_ROT) / 2) + _body_x_add) * 2.5,
			Y1=(y + (_body_y / 2) - (sprite_height / 2) - lengthdir_y(sprite_height / 2, -rot + (VAR_ROT / 2)) + _body_y_add) * 2.5,
			X2=(xstart - lengthdir_x(512, -rot + VAR_ROT)) * 2.5,
			Y2=(ystart - 25 - (sprite_height / 2) - lengthdir_y(512, -rot + VAR_ROT)) * 2.5,
			VAR_DIR=point_direction(X1,Y1,X2,Y2);
		draw_line_width(X1,Y1,X2,Y2, 4 * 2.5);
		draw_sprite_ext(spr_cellien_big_monster_tentacle_start, 0, X1, Y1, 1 * 2.5, 1 * 2.5, VAR_DIR, c_white, image_alpha);
	}
...
if (_draw_paperplane) {
	var INST=id;
	if(instance_exists(_paperplane)&&_paperplane!=-1&&_paperplane!=battle_enemy_cellien_big){
		with(_paperplane){
