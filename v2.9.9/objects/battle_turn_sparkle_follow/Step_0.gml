	...
	if (_fade_all) {
		with (inst) {
			if (image_alpha > 0)
				...
				battle_enemy_cellien_small._dodged = true;
				instance_destroy(battle_turn_sparkle_follow);
				instance_destroy();
			}
		}
	}
...
if(!instance_exists(_inst))&&(!_fade_all){
	instance_destroy(battle_bullet_sparkle);
	Cutscene_End(-1);
	Cutscene_Begin();
	battle_enemy_cellien_small._phase = 4;
	battle_enemy_cellien_small._dodged = false;
	instance_destroy();
}
