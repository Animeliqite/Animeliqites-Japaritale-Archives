...
if(_state==BATTLE_STATE.MENU){
		...
		if(_menu_can_move){
			...
			if(Input_IsPressed(INPUT.CONFIRM)){
                if(!_menu_button_cutscene_func){
                    ...
                    if (_menu_button_cutscene_func_method != undefined)
                        method_call(_menu_button_cutscene_func_method);
                }
			}
		}
	}else
	...
	if(_menu==BATTLE_MENU.ITEM_ACTION){
		//上/下
		if(Input_IsPressed(INPUT.UP)){
			var slot=_menu_choice_item_action-1;
			if(slot>=0){
				audio_play_sound_on(global._gmu_emitter_sfx,snd_menu_switch,0,false);
				Battle_SetMenuChoiceItemAction(slot);
			}
		}else if(Input_IsPressed(INPUT.DOWN)){
			var slot=_menu_choice_item_action+1;
			if(slot<array_length(_item_action)){
				audio_play_sound_on(global._gmu_emitter_sfx,snd_menu_switch,0,false);
				Battle_SetMenuChoiceItemAction(slot);
			}
		}else if(Input_IsPressed(INPUT.CANCEL)&&_item_action_cancellable){
			Battle_SetMenu(BATTLE_MENU.BUTTON);
		}else if(Input_IsPressed(INPUT.CONFIRM)){
			battle_soul.x=battle_board.x;
			battle_soul.y=battle_board.y;
			audio_play_sound_on(global._gmu_emitter_sfx,snd_menu_confirm,0,false);
			_item_action[_menu_choice_item_action].onUse();
		}
		
		if(!Input_IsPressed(INPUT.CONFIRM)){
			battle_soul.x=battle_board.x-battle_board.left-5+40;
			battle_soul.y=battle_board.y-battle_board.up-5+36+32*(_menu_choice_item_action-_menu_choice_item_action_first);
		}
	}else
...
if(_state==BATTLE_STATE.TURN_PREPARATION){
	if(Battle_IsTurnPreparationAutoEnd()){
		if(!instance_exists(battle_dialog_enemy) && !Battle_IsBoardTransforming() && !global._gmu_cutscene){
...
if(_state!=BATTLE_STATE.RESULT && Battle_GetEnemyNumber()==0){
	...
	if(Player_UpdateLv()){
		...
		audio_play_sound_on(global._gmu_emitter_sfx,snd_level_up,0,false);
	}
	text+="{pause}{end}";
