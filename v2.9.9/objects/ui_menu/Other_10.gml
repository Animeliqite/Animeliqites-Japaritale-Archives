if(_menu==0.5||_menu==1){
	instance_destroy(_inst_item);
	if(!instance_exists(_inst_item)){
		_inst_item=instance_create_depth(188+6+38,52+6+80,0,text_typer);
		_inst_item.text=_prefix;
		var proc=0;
		repeat(Item_GetNumberIn(_item_type)){
			_inst_item.text+=Item_GetName(Item_Get(proc,_item_type))+"&";
			proc+=1;
		}
	}
