...
if(_menu==-1){
	...
	if(Input_IsPressed(INPUT.LEFT)){
		if(_choice_item_type>0){
			_choice_item_type-=1;
			_item_type=(_choice_item_type==0 ? "ITEM" : (_choice_item_type==1 ? "TOOL" : "KEY"));
			event_user(0);
			audio_play_sound_on(global._gmu_emitter_sfx,snd_menu_switch,0,false);
		}
	}else if(Input_IsPressed(INPUT.RIGHT)){
		if(_choice_item_type<2){
			_choice_item_type+=1;
			_item_type=(_choice_item_type==0 ? "ITEM" : (_choice_item_type==1 ? "TOOL" : "KEY"));
			event_user(0);
			audio_play_sound_on(global._gmu_emitter_sfx,snd_menu_switch,0,false);
		...
		if(Item_GetNumberIn(_item_type)>0){
			_menu=1;
			event_user(0);
		}
	...
	if(Input_IsPressed(INPUT.UP)){
		...
		if(_choice_item<Item_GetNumberIn(_item_type)-1){
			_choice_item+=1;
			audio_play_sound_on(global._gmu_emitter_sfx,snd_menu_switch,0,false);
		}
	}else if(Input_IsPressed(INPUT.CONFIRM)){
	...
	if(Input_IsPressed(INPUT.LEFT)){
		...
		switch(_choice_item_operate){
			case 0:
				Item_CallEvent(Item_Get(_choice_item,_item_type),ITEM_EVENT.USE,_choice_item);
				break;
			case 1:
				Item_CallEvent(Item_Get(_choice_item,_item_type),ITEM_EVENT.INFO,_choice_item);
				break;
			case 2:
				Item_CallEvent(Item_Get(_choice_item,_item_type),ITEM_EVENT.DROP,_choice_item);
				break;
		}
		audio_play_sound_on(global._gmu_emitter_sfx,snd_menu_confirm,0,false);
	}else if(Input_IsPressed(INPUT.CANCEL)){
