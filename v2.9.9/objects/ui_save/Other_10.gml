if(_state==0){
	...
	_inst_name.text=_prefix+Flag_Get(FLAG_TYPE.INFO,FLAG_INFO.USERNAME,Lang_GetString("ui.save.name.empty"));
	...
	_inst_room.text=_prefix+Player_GetRoomName(asset_get_index(Flag_Get(FLAG_TYPE.INFO,FLAG_INFO.ROOM,-1)));
...
if(_state==1){
	...
	_inst_name=instance_create_depth(147+45,163+110,0,text_typer);
	_inst_name.text=_prefix+"{color_text `yellow`}"+Flag_Get(FLAG_TYPE.INFO,FLAG_INFO.USERNAME,Lang_GetString("ui.save.name.empty"));
