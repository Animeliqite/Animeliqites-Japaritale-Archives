...
if(_skippable&&!_paused&&Input_IsPressed(INPUT.CANCEL)&&_char_amt>0){
	_skipping=true;
	_sleep=0;
...
if(instance_exists(_face)){
	_face.gui=_gui;
	_face.depth=depth-20;
	_face.talking=(!_paused&&_char_proc<string_length(text));
}

if(_face_linked!=-1){
	if(instance_exists(face)){
		var fid=_face_linked;
		var ftalking=(!_paused&&_char_proc<string_length(text));
		with(face){
			if(face_id==fid){
				talking=ftalking;
			}
...
if(_char_linked!=-1){
	if(instance_exists(char)){
		...
		var ctalking=(!_paused&&_char_proc<string_length(text));
		with(char){
			if(char_id==cid){
				if(!ctalking) talking_as=false;
				else talking_as=true;
				talking=ctalking;
			}
		}
	}
