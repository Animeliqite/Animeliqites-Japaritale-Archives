...
switch(cmd[|0]){
		...
		if(is_string(cmd[|1])){
			...
			switch(color){
				...
				case c_red:
					...
					break;
				case c_fuchsia:
					_color_text[0]=make_color_rgb(255,215,255);
					_color_text[1]=make_color_rgb(255,215,255);
					_color_text[2]=make_color_rgb(229,114,229);
					_color_text[3]=make_color_rgb(229,114,229);
					_color_shadow[0]=make_color_rgb(64,42,64);
					_color_shadow[1]=make_color_rgb(64,42,64);
					_color_shadow[2]=make_color_rgb(64,42,64);
					_color_shadow[3]=make_color_rgb(64,42,64);
					break;
			}
		}
		break;
	...
	case "sound":
		...
		if(audio_exists(target)){
			audio_play_sound_on(global._gmu_emitter_sfx,target,0,false);
		}
		break;
	case "top":
		if(is_bool(cmd[|1])){
	...
	case "char_dir":
		if(is_real(cmd[|1])&&is_real(cmd[|2])){
			if(instance_exists(char)){
				var cid=cmd[|1];
				var cdir=cmd[|2];
				var change_upon_stop=false;
				
				if(is_real(cmd[|3])){
					change_upon_stop=cmd[|3];
				}
	...
	case "char_move":
		if(is_real(cmd[|1])&&is_real(cmd[|2])&&is_real(cmd[|3])&&is_bool(cmd[|4])){
			if(instance_exists(char)){
				var cid=cmd[|1];
				var cdir=cmd[|2];
				var cmove=cmd[|3];
				...
				with(char){
					if(char_id==cid){
						move[cdir]=cmove;
						move_moveable=cmoveable;
						in_move_cutscene=cmoveable;
					}
				}
			}
		}
	...
	case "subtext":
		if(is_string(cmd[|1])){
			array_push(_subtext,Typer_Type(x+_char_x,y+_char_y-10,cmd[|1],"{gui true}{voice -1}{font 2}"));
			show_debug_message("works!")
		}
		break;
		
	case "char_move_new":
		if(is_real(cmd[|1])&&is_real(cmd[|2])&&is_real(cmd[|3])&&is_real(cmd[|4])&&is_real(cmd[|5])&&is_bool(cmd[|6])){
			var cid=cmd[|1];
			var relative=cmd[|6];
				
			for (var i=0; i<instance_number(char); i++){
				if(instance_find(char,i).char_id==cid){
					MoveChar(instance_find(char,i),cmd[|3],cmd[|4],relative,cmd[|5],false,cmd[|2]);
				}
			}
			show_debug_message("Works!");
		}
	...
	case "char_caterpillar":
		if(is_real(cmd[|1])&&is_bool(cmd[|2])){
			if(instance_exists(char)){
				with (char) {
					if (char_id==cmd[|1]){
						caterpillar=cmd[|2];
