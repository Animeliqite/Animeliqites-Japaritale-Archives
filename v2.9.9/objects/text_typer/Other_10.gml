...
if(_char!=" "&&_char!="　"){
	...
	var INST=instance_create_depth(x+_char_x+_offset_x*(1+_group_font_offset_x[_font,font]),y+_char_y+_offset_y*(1+_group_font_offset_y[_font,font])+OFFSET,depth,text_single);
	INST.text=_char;
	INST.siner=_siner;
	...
	if(!_voice_played&&!_skipping&&!_instant&&_voice>=0){
		...
		if(audio_exists(sound)){
			...
			audio_play_sound_on(global._gmu_emitter_sfx,sound,0,false);
			_voice_played=true;
			_voice_timer=1;
		}
	}
